    ...
    with open(pth, 'r') as file:
        for line in file:
            ...
            if len(parts) == 2:
                ...
                # Check if the key and value conform to the specified rules
                ticker_exchange_dict[ticker] = exchange

    return ticker_exchange_dict
...
def read_dat(tic):
    ...
    # Construct the full path to the .dat file based on the `tic` and `dat_dir`
    filename = os.path.join(DATDIR, f'{tic}_prc.dat')

    try:
        # Open the .dat file for the specified ticker in read mode
        with open(filename, 'r') as file:
            # Read each line and append it to the 'lines' list
            for line in file:
                lines.append(line.strip())  # Remove newline characters

        return lines  # Return the list of lines from the .dat file

    except FileNotFoundError:
        print(f"File '{filename}' not found for ticker '{tic}'")
...
def line_to_dict(line):
    ...
    COLUMNS_ALL = ['Volume', 'Date', 'Adj Close', 'Close', 'Open', 'High']
    COLWIDTHS_ALL = {'Volume': 14, 'Date': 11, 'Adj Close': 19, 'Close': 10, 'Open': 6, 'High': 20}
    # Define the starting position for the first column
    start = 0

    # Iterate through each column in COLUMNS_ALL
    for col in COLUMNS_ALL:
        # Get the width of the current column from  COLWIDTHS_ALL

        width = COLWIDTHS_ALL[col]
        ...
        data_dict_ALL[col] = value

        # Update the starting position for the next column
        start += width

    for col in COLUMNS:
        data_dict[col] = data_dict_ALL[col]
    return data_dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    # Rule 2: Check if tickers_lst contains tickers that are not in tic_exchange_dic
    if tickers_lst is not None:
        invalid_tickers = [ticker for ticker in tickers_lst if ticker.lower() not in tic_exchange_dic]
        if invalid_tickers:
            raise ValueError(f"Invalid tickers found in the list: {', '.join(invalid_tickers)}")
...
def verify_cols(col_lst=None):
    ...
    # Rule 2: Check if col_lst contains columns that are not found in COLUMNS
    if col_lst is not None and COLUMNS is not None:
        invalid_cols = [col for col in col_lst if col.lower() not in map(str.lower, COLUMNS)]
        if invalid_cols:
            raise ValueError(f"Invalid column names found in the list: {', '.join(invalid_cols)}")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # If tickers_lst is None, consider all tickers from tic_exchange_dic
    ...
    # If col_lst is None, consider all columns from COLUMNS
    if col_lst is None:
        col_lst = COLUMNS

    # Verify if the provided column names are valid
    verify_cols(col_lst)

    # Iterate through tickers and collect data
    for ticker in tickers_lst:
        ...
        # Read data for the ticker and extract columns from col_lst
        for line in read_dat(ticker):
            data_dict_line = line_to_dict(line)
            ticker_data.append(data_dict_line)

        data_dict[ticker] = {'exchange': exchange.lower(), 'data': ticker_data}
